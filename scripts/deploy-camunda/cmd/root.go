...
func NewRootCommand() *cobra.Command {
	...
	f.StringVar(&flags.Selection.Persistence, "persistence", "", "Persistence selection: elasticsearch, elasticsearch-self-signed, elasticsearch-external-self-signed, opensearch, opensearch-embedded, opensearch-external, opensearch-self-signed-os-trust, rdbms, rdbms-external, rdbms-oracle")
...
func registerSelectionCompletion(cmd *cobra.Command) {
	...
	_ = cmd.RegisterFlagCompletionFunc("persistence", func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		scenarioPath := resolveScenarioPath(cmd)
		defaultPersistence := []string{"elasticsearch", "opensearch", "opensearch-self-signed-os-trust", "rdbms", "rdbms-external", "rdbms-oracle"}
